...
app.set('port', process.env.PORT || commonConfig.dpMountStorageAppPort);
		...
		execFile('/bin/mount', arg, function(err, stdout, stderr) {
	  	if (err || stderr) {
				if (err) {
						...
						resultCallback.sendHttpRequest(commonConfig.jsonContentType, postData, commonConfig.dpMountStorageResultEP, reqIp, commonConfig.treatmentControllerPort);
				} else {
